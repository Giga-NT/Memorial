<<<<<<< HEAD
# app.py - ПОЛНАЯ ВЕРСИЯ С ЗАГРУЗКОЙ 3D МОДЕЛЕЙ
...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'stela-secret-key-2024-change-in-production'
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///stela.db'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
app.config['MAX_CONTENT_LENGTH'] = 50 * 1024 * 1024  # 50MB для 3D моделей
...
class Project(db.Model):
    ...
    mode = db.Column(db.String(50), default='engraving')
    ...
    original_image = db.Column(db.Text)
    enhanced_image = db.Column(db.Text)
    engraving_image = db.Column(db.Text)
    mask_image = db.Column(db.Text)
...
def send_order_email(project):
    try:
        subject = f"📋 Заказ #{project.id} - {project.client_name or 'Новый заказ'}"
        
        body = f"""
📋 НОВЫЙ ЗАКАЗ #{project.id}
{'='*50}

👤 КЛИЕНТ:
ФИО: {project.client_name or 'Не указано'}
Телефон: {project.client_phone or 'Не указано'}
Email: {project.client_email or 'Не указано'}

🗿 ПАРАМЕТРЫ:
Режим: {'Гравировка' if project.mode == 'engraving' else 'Фото-декаль'}
Камень: {project.stone or 'Классический'}
Форма: {project.shape or 'Без формы'}
Масштаб: {project.scale or '0.8'}

🎨 ОБРАБОТКА:
Глубина: {project.depth or '0.9'}
Ореол: {project.halo or '35'}
Контраст: {project.contrast or '3.0'}
Яркость: {project.brightness or '0'}
...
def save_project():
    try:
        ...
        project.mode = data.get('mode', 'engraving')
        project.shape = data.get('shape', 'none')
        project.stone = data.get('stone', 'classic')
        project.texture = data.get('texture', '')
        project.blur = float(data.get('blur', 3))
        project.depth = float(data.get('depth', 0.9))
        project.halo = int(data.get('halo', 35))
        project.contrast = float(data.get('contrast', 3.0))
        project.brightness = int(data.get('brightness', 0))
        project.scale = float(data.get('scale', 0.8))
        project.offset_x = float(data.get('offset_x', 0))
        project.offset_y = float(data.get('offset_y', 0))
        project.offset_z = float(data.get('offset_z', -0.1))
        project.crop_top = float(data.get('crop_top', 2))
        project.crop_bottom = float(data.get('crop_bottom', 2))
        project.crop_left = float(data.get('crop_left', 2))
        project.crop_right = float(data.get('crop_right', 2))
        project.photo_mode = data.get('photo_mode', 'color')
        project.photo_brightness = int(data.get('photo_brightness', 0))
        project.photo_contrast = float(data.get('photo_contrast', 1.0))
        project.notes = data.get('notes', '')
        
        if data.get('original_image'):
            project.original_image = data['original_image']
        if data.get('enhanced_image'):
            project.enhanced_image = data['enhanced_image']
        if data.get('engraving_image'):
            project.engraving_image = data['engraving_image']
        if data.get('mask_image'):
            project.mask_image = data['mask_image']
...
def get_project(project_id):
    ...
    return jsonify({
        ...
        'project': {
            ...
            'crop_bottom': project.crop_bottom,
            'crop_left': project.crop_left,
            'crop_right': project.crop_right,
            'photo_mode': project.photo_mode,
            'photo_brightness': project.photo_brightness,
            'photo_contrast': project.photo_contrast,
            'original_image': project.original_image,
            'enhanced_image': project.enhanced_image,
            'engraving_image': project.engraving_image,
            'mask_image': project.mask_image,
            'notes': project.notes,
    ...
    try:
        ...
        if new_status:
            ...
            if new_status == 'printed':
                project.printed_at = datetime.utcnow()
...
def download_pdf(project_id):
    try:
        ...
        pdf_buffer = generate_order_pdf(project, project.engraving_image)
        
        return send_file(
            pdf_buffer,
            as_attachment=True,
            download_name=f'заказ_{project.id}_{datetime.now().strftime("%Y%m%d")}.pdf',
...
def guest_create_order():
    try:
        ...
        project = Project(user_id=admin.id if admin else 1)
...
if __name__ == '__main__':
    print("🚀 Запуск сервера гравировки...")
